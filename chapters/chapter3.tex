...
\textbf{Notice that code snippets above involving maths and circuits is untested and is highly error-prone.}

\section{Exercise 3.81}

Code:

\begin{lstlisting}
(define (new-rand init)
  (let ((s (cons-stream init
                        (stream-map rand-update s))))
    (cons s s)))

(define (generate rand)
  (cons (stream-car (car rand))
        (cons (stream-cdr (car rand))
              (cdr rand))))

(define (reset rand)
  (cons (cdr rand) (cdr rand))) 
\end{lstlisting}

\section{Exercise 3.82}
...
(define (create-rand seed)
  (define (rand-next x)
    (modulo (* x 16807) 2147483647))
  (stream-map (lambda (x) (/ x 2147483647.0))
              (make-stream (rand-next seed)
                           rand-next
                           (lambda (x) x))))
...
(define (monte-carlo experiments passed failed)
  (cons-stream (/ passed (+ passed failed))
               (if (stream-car experiments)
                   (monte-carlo (stream-cdr experiments)
                                (+ passed 1)
                                failed)
                   (monte-carlo (stream-cdr experiments)
                                passed
...
(define (estimate-integral predicate xlow xup ylow yup)
  (define (rand-range lower upper seed)
    (stream-map (lambda (x) (+ (* (- upper lower)
                                  x)
                               lower))
                (create-rand seed)))
  (define experiments
    (stream-map predicate
                (rand-range xlow xup 233)
                (rand-range ylow yup 2333)))
  (monte-carlo experiments 1 0))

(define pi-estimate
  (stream-map (lambda (x) (* x 4.0))
              (estimate-integral
                (lambda (x y)
                  (< (+ (* x x) (* y y)) 1))
                -1.0 1.0 -1.0 1.0)))
...
\begin{lstlisting}
(stream-ref pi-estimate 10)
(stream-ref pi-estimate 100)
(stream-ref pi-estimate 1000)
(stream-ref pi-estimate 10000)
(stream-ref pi-estimate 100000)
(stream-ref pi-estimate 1000000)
\end{lstlisting}
